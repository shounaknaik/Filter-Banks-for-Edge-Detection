...
def TrainOperation(TrainLabels, NumTrainSamples, ImageSize,
    ...
    for Epochs in tqdm(range(StartEpoch, NumEpochs)):
        ...
        for PerEpochCounter in tqdm(range(NumIterationsPerEpoch)):
            ...
            LossThisBatch, accuracy = model.training_step(Batch)

            batch_wise_accuracy.append(accuracy)
            batch_wise_loss.append(LossThisBatch)
        ...
        epoch_wise_accuracy.append(np.mean(batch_wise_accuracy))
        epoch_wise_loss.append(np.sum(batch_wise_loss))
    ...
    plt.figure()
    plt.plot(range(len(epoch_wise_accuracy)),epoch_wise_accuracy)
    plt.title('Train Accuracy over epochs')
    plt.xlabel('Epochs')
    plt.ylabel('Train Accuracy')
    plt.savefig('Accuracy_improved.png')
    
    plt.figure()
    plt.plot(range(len(epoch_wise_loss)),epoch_wise_loss)
    plt.title('Loss over epochs')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.savefig('Loss_improved.png')
...
def main():
    ...
    Parser.add_argument('--BasePath', default='../CIFAR10/Train/', help='Path to image data Default: ../CIFAR10/Train/')
